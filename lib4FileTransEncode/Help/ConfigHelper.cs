...
namespace _93000.FileTranscoding.Base.FWCL.Help
{
    ...
    public class ConfigHelper
    {
        ...
        public static void UpdateConfig<T>(T config)
        {
            ...
            string configPath = GetConfigPath<T>(); //根据配置文件名读取配置文件  
            try
            {
                var xmlSerializer = new XmlSerializer(classType);
                using (var xmlTextWriter = new XmlTextWriter(configPath, Encoding.UTF8))
                {
        ...
        {
            var dirpath = Path.Combine(Thread.GetDomain().BaseDirectory, "Config");
            if (!Directory.Exists(dirpath))
            {
                Directory.CreateDirectory(dirpath);
            }
            dirpath = Path.Combine(dirpath, typeof(T).Name + ".config");
            return dirpath;
        }
